...
void Block::clear(){
    ...
    for(int i{0}; i<20; ++i){
        if (memcmp(screen[i],compare,10) == 0){
            for (int y{0}; y<19; ++y){
                if (19-y <= i){
                    memcpy(screen[19-y], screen[18-y], 10);
                }
            }
        }   
    }
...
void Block::update(){
    int BLOCKS[7] {78, 142, 198, 102, 108, 46, 15}; // each digit contains a block and what it looks like

    auto start{std::chrono::high_resolution_clock::now()};
    auto end{std::chrono::high_resolution_clock::now()};
    
    while (1){
        // check game over and create block
        ...
        while(1){
            ...
            if (stop_block > 3){         //freezing block 
                stop_block = 0;
                freeze_block();
                clear();
                break;
